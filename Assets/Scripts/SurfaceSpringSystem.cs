...
public class SurfaceSpringSystem : MonoBehaviour
{
    ...
    public SurfaceSpringSystem otherSystem;
    ...
    void OnDrawGizmos()
    {
        ...
        #if UNITY_EDITOR
        AABB aabb = new AABB(massPoints.Concat(internalPoints).ToList());
        Gizmos.color = Color.yellow;
        Gizmos.DrawWireCube(aabb.center, aabb.extents * 2f);
        #endif
    }
    ...
    void DetectAndResolveCollision()
    {
        if (otherSystem == null)
        {
            Debug.LogWarning("⚠️ otherSystem is not assigned.");
            return;
        }

        AABB aabbA = new AABB(this.massPoints.Concat(this.internalPoints).ToList());
        AABB aabbB = new AABB(otherSystem.massPoints.Concat(otherSystem.internalPoints).ToList());

        Debug.Log($"📦 Checking AABB collision between {name} and {otherSystem.name}");

        if (aabbA.Intersects(aabbB))
        {
            Debug.Log("✅ AABB overlap detected. Proceeding to GJK...");

            bool realCollision = GJK.CheckCollision(
                this.massPoints.Concat(this.internalPoints).ToList(),
                otherSystem.massPoints.Concat(otherSystem.internalPoints).ToList()
            );


            if (realCollision)
            {
                Debug.Log("💥 GJK confirmed collision! Applying physical response.");

                Vector3 separationVector = (aabbB.center - aabbA.center).normalized * 0.05f;
                float transferRatio = 0.4f;
                float maxTransferDistance = 0.7f;

                ApplyCollisionResponse(this.massPoints, this.internalPoints, -separationVector, transferRatio, maxTransferDistance, true);
                ApplyCollisionResponse(otherSystem.massPoints, otherSystem.internalPoints, separationVector, transferRatio, maxTransferDistance, false);
            }
            else
            {
                Debug.Log("🚫 GJK rejected collision (false positive in AABB).");
            }
        }
        else
        {
            Debug.Log("❌ No collision (AABB rejected).");
        }
    }
    ...
    void ApplyCollisionResponse(List<MassPoint> surfacePoints, List<MassPoint> internalPoints, Vector3 separationVector, float transferRatio, float maxTransferDistance, bool isMain)
    {
        float maxForceMagnitude = 200f; 
        float pushForceMultiplier = 50f;

        foreach (var mp in surfacePoints)
        {
            Vector3 impactForce = -mp.velocity * restitution;
            Vector3 totalForce = separationVector * pushForceMultiplier + impactForce;

            if (totalForce.magnitude > maxForceMagnitude)
                totalForce = totalForce.normalized * maxForceMagnitude;

            mp.AddForce(totalForce * (1 - transferRatio));
        }

        foreach (var mp in internalPoints)
        {
            float distanceSqr = (mp.position - Vector3.zero).sqrMagnitude;
            if (distanceSqr <= maxTransferDistance * maxTransferDistance)
            {
                Vector3 forceDir = isMain ? -separationVector : separationVector;
                Vector3 force = forceDir * (1 - transferRatio) * 0.5f;
                mp.AddForce(force);
            }
        }
    }
